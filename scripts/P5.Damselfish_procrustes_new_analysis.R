...
lmk_names[lmk_names == "pomacentrus-maafu_1"] = "Pomacentrus-maafu_1"
img_names[img_names == "pomacentrus maafu_semanticscholar2"] = "Pomacentrus maafu_semanticscholar2"
...
mispelled_lmk
sp_names[sp_names == "Chrysiptera_maurinae"] = "Chrysiptera_maurineae"
sp_names[sp_names == "Plectroglyphidodon_leucozona"] = "Plectroglyphidodon_leucozonus"
sp_names[sp_names == "Pomacentrus_caeruleopuncatus"] = "Pomacentrus_caeruleopunctatus"
sp_names[sp_names == "Pycnochromis_lineata"] = "Pycnochromis_lineatus"
...
# plot gpa landmarks
plot(NULL, 
     xlim = range(pretty(apply(all_temp$coords, 3, function(x) range(x[,1])))),
     ylim = range(pretty(apply(all_temp$coords, 3, function(x) range(x[,2])))))
apply(all_temp$coords, 3, function(i) points(i, pch = 19, col = rainbow(26)))
text(apply(apply(all_temp$coords, 1, function(i) i[1:dim(all_temp$coords)[3] %% 2 != 0]), 2, mean),
     apply(apply(all_temp$coords, 1, function(i) i[1:dim(all_temp$coords)[3] %% 2 == 0]), 2, mean),
       1:26)

all_temp <- gpagen(Ldmks_array[,,])

all_m.temp <- matrix(NA,dim(Ldmks_array)[3],12) # 11 corresponds to the number of traits to be recorded...
...
for(j in 1:dim(Ldmks_array)[3]){
  ...
  # 8 eye vertical position relative to body depth (most ventral point)
  all_eye.centroid<- apply(rbind(get_point(1,j),
                                 get_point(2,j),
                                 get_point(3,j),
                                 get_point(4,j)),2,mean)
  all_eye_height <- mean(all_dist.p(all_eye.centroid,20,j), all_dist.p(all_eye.centroid,18,j))
  all_m.temp[j,8] <- all_eye_height
  ...
  # 11 peduncle_depth max(23->31 and 24->30)
  all_m.temp[j,11]<- max(all_dist.p(14,17,j), all_dist.p(15,16,j))
  
  # 12 peduncle_length max(23->24 and 30->31)
  all_m.temp[j,12]<- max(all_dist.p(14,15,j), all_dist.p(16,17,j))
  
}
all_m <- all_m.temp

### To extract trait values from landmarks ###
all_morpho_ratio <- data.frame(body_ratio=all_m[,6]/all_m[,4], 
                               head_ratio=all_m[,1]/all_m[,4], 
                               snout_ratio=all_m[,2]/all_m[,1], 
                               eye_ratio=all_m[,3]/all_m[,1], 
                               peduncle_ratio=all_m[,9]/all_m[,6],
                               anal_fin_ratio=all_m[,5]/all_m[,4], 
                               pelvic_fin_ratio=all_m[,10]/all_m[,4], 
                               eye_height=all_m[,8]/all_m[,6], 
                               snout_angle=all_m[,7], 
                               peduncle_depth_factor=all_m[,11]/(all_m[,11] * all_m[,12]), 
                               relative_body_length=all_m[,4])

# plot morpho ratios
par(mfrow = c(3,4))
sapply(colnames(all_morpho_ratio), function(i) plot(all_morpho_ratio[,i], ylab = i))
...
par(mfrow = c(3,4))
sapply(colnames(all_morpho_ratio_sp), function(i) plot(all_morpho_ratio_sp[tree$tip.label,i], ylab = i))
...
eco_traits_subset <- eco_traits_subset[, c(1:3, 7:ncol(eco_traits_subset), 4:6)]
