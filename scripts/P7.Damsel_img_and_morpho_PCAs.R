...
imgs_path <- "./data/images/TransIntImgPNGColorCorrectedMasked/"
img_filenames <- list.files(imgs_path, full.names = TRUE)
...
# mask background
Rmask <- raster::stack("./data/mask.png")[[4]]
Rmask[Rmask == 0] = NA
Rmask[Rmask > 0] = 1
Rmask <- raster::flip(Rmask, "y")

imgTransInt.list_res300 <- makeList(img_names, 'image', prepath = imgs_path, extension = ".png",
                                    layers = 1:3, flip = "y", mask = Rmask)
...
species[!species %in% tree_species] # 12 missing species in the tree
tree_species[!tree_species %in% species] # 0 missing species from image data


# Reorder species based on the tree 
species <- intersect(tree_species, species)
n_sp <- length(species)

# Assign color to species
genus <- sapply(species, function(i) strsplit(i, "_")[[1]][1])
names(genus) <- species
genus_cols_legend <- setNames(colorRampPalette(viridis::viridis(9))(n=length(unique(genus))),
                              unique(genus))
...
imgPCA_1183damsel <- imagePCA(imgTransInt.list_res300, tree = NULL, pcs = 1:8, impute.NA = TRUE, 
                              res = NULL, interpolate = FALSE, scale = TRUE, plot = FALSE) 

saveRDS(imgPCA_1183damsel, "./rdata/imgPCA_1183damsel_scaled.RDS")

imgPCA_1183damsel <- readRDS("./rdata/imgPCA_1183damsel_scaled.RDS")
...
for (pc in c(1, 3, 5, 7)) {
  ...
  pdf(paste0("./figures/1183imgs_PC", pc, pc +1, ".pdf"), width = 14, height = 11)
...
colpc_legend_images(imgPCA = imgPCA_1183damsel, pcs = 1:8, col = "grey40", cex_main = 0, cex_text = 4.5,
                    out_dir="./figures/pca_legends")
...
for (pc in c(1, 3, 5, 7)) {
  ...
  pdf(paste0("./figures/1183imgs_PC", pc, pc +1, "_spmean.pdf"), width = 14, height = 11)
...
write.csv(species_pcdata, "./data/1188img_colorPCA_spmean.csv", row.names = F)
...
colSums(apply(sp_morpho, 2, is.na))
...
for (pc in c(1, 3, 5, 7)) {
  ...
  pdf(paste0("./figures/morphoPCA_", pcx, pcy, ".pdf"), width = 14, height = 11)
...
morphopc_legend_images(morphoPCA$x, morpho_gpa$coords, sp_names,
                       show.grid = FALSE, cex_text = 2.5,
                       show.outline = TRUE, show.points = FALSE, 
                       col_body = "grey20", col_eye = "white",
                       fill_body = "grey20", fill_eye = "white", lwd_body = 5,
                       lwd_bone=1, col_preop="grey80", col_opercle="grey80",
                       pcs = 1:8, out_dir="./figures/pca_legends")
